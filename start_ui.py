...
class BestApp(App):
    ...
    def __init__(self, **kwargs):
        ...
        if SEND_SQL:
            self.class_main.class_db.connect(host=self.setting['db_host'],
                                             database=self.setting['db_database'],
                                             user=self.setting['db_user'],
                                             password=self.setting['db_password'])
            self.class_main.sql_table = self.setting['db_table']

        thread.start_new_thread(self.class_main.send_query, ())
        for i in range(len(self.class_main.cap_list)):
            thread.start_new_thread(self.class_main.read_frame, (i, RESIZE_FACTOR))
            thread.start_new_thread(self.class_main.process_frame, (i, ))
    ...
    def on_cam_set(self, cam1, cam2, cam3, cam4, cam5, cam6, cam7, cam8, host, port, user, password, database, table):
        ...
        if f_change_db:
            ...
            self.class_main.enable_send_query = False
            self.class_main.class_db.disconnect()
            self.class_main.class_db.connect(host=host, database=database, user=user, password=password)
            self.class_main.sql_table = table
            self.class_main.enable_send_query = True
